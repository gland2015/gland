...
import  SampleDraftInlineStyle from '../../model/immutable/SampleDraftInlineStyle';
const {BOLD} = SampleDraftInlineStyle;
...
const getSampleSelectionMocksForTesting = (): Object => {
  ...
  const aChars = List(
    Repeat(EMPTY, text[0].length).concat(
      Repeat(boldChar, text[1].length),
    ),
  );

  const bChars = List(
    Repeat(EMPTY, text[2].length).concat(
      Repeat(boldChar, text[3].length),
    ),
  );

  const cChars = List(
    Repeat(EMPTY, text[4].length).concat(
      Repeat(boldChar, text[5].length),
    ),
  );
...
export default  getSampleSelectionMocksForTesting;
