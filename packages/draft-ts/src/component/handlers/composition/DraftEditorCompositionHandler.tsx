...
const DraftEditorCompositionHandler = {
    ...
    resolveComposition: function (editor: DraftEditor): void {
        ...
        mutations.forEach((composedChars, offsetKey) => {
            ...
            const { start, end } = editorState.getBlockTree(blockKey).getIn([decoratorKey, "leaves", leafKey]);
            ...
            const entityKey = getEntityKeyForSelection(contentState, replacementRange);
            const currentStyle = contentState.getBlockForKey(blockKey).getInlineStyleAt(start);

            contentState = DraftModifier.replaceText(contentState, replacementRange as any, composedChars, currentStyle, entityKey);
        ...
        const editorStateWithUpdatedSelection = EditorState.acceptSelection(editorState, compositionEndSelectionState);
