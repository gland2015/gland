...
import DraftTreeOperations from './DraftTreeOperations';
...
const NestedRichTextEditorUtil: RichTextUtils = {
  ...
  toggleBlockType: (
    ...
    const selection = editorState.getSelection();
    const content = editorState.getCurrentContent();
    const currentBlock:any = content.getBlockForKey(selection.getStartKey());
  ...
  onTab: (
    ...
    if (!event.shiftKey) {
      ...
      const prevSibling:any = blockMap.get(prevSiblingKey);
      const nextSibling:any =
      ...
      blockMap = onUntab(blockMap, block as any);
    }
    ...
    if (selection.isCollapsed() && offset === 0) {
      ...
      if (type !== 'unstyled') {
        if (
          ...
          let newBlockMap:any = onUntab(content.getBlockMap(), block as any);
          newBlockMap = newBlockMap.set(
            key,
            newBlockMap.get(key).merge({depth: depth - 1}),
          );
          return content.merge({blockMap: newBlockMap}) as any;
        }
...
const onUntab = (blockMap: BlockMap, block: ContentBlockNode): BlockMap => {
  ...
  if (blockIndex === 0 || blockIndex === existingChildren.count() - 1) {
    ...
    blockMap = blockMap
      .set(key, block.merge({nextSibling: null}) as any)
      .set(
        nextSiblingKey,
        blockMap.get(nextSiblingKey).merge({prevSibling: null}) as any,
      );
  ...
  if (parentKey != null) {
    let parent:any = blockMap.get(parentKey);
    while (parent != null) {
      const children:any = parent.getChildKeys();
      const firstChildKey = children.first();
      invariant(firstChildKey != null, 'parent must have at least one child');
      const firstChild:any = blockMap.get(firstChildKey);
